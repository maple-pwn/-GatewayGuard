...
def _risk_summary(total: int, weighted_score: float, critical_count: int, high_count: int, medium_count: int) -> dict:
    if total <= 0:
        return {"riskLabel": "CLEAR", "riskScore": 0, "riskHint": "暂无异常事件"}

    risk_score = round((weighted_score / (total * 4)) * 100)
    if critical_count:
        return {"riskLabel": "CRITICAL", "riskScore": risk_score, "riskHint": "存在严重风险事件，建议立即处置"}
    if high_count:
        return {"riskLabel": "HIGH", "riskScore": risk_score, "riskHint": "存在高危事件，建议优先调查"}
    if medium_count:
        return {"riskLabel": "MEDIUM", "riskScore": risk_score, "riskHint": "当前以中低风险事件为主"}
